...
- 充分考慮${companyProfile.sizeText}企業在${companyProfile.industryText}的實際執行能力
- 提供具體的數字估算和時間框架
- 語言專業但易懂，適合企業管理層閱讀
- 使用繁體中文
- 每個面向約40-60字，避免過於冗長
...
請以JSON格式回傳，格式如下：
{
  ...
  try {
    ...
    return {
      analysis: {
        profitLossAnalysis: `針對${subcategoryName}情境執行${strategyName}，預期對${companyProfile.sizeText}${companyProfile.industryText}企業營收產生正面影響，估計可創造3-8%的營收成長機會。`,
        cashFlowAnalysis: `執行${strategyName}需要分階段資金投入，初期投資約200-500萬元，預計3-5年回收，可透過政府補助管理現金流壓力。`,
        balanceSheetAnalysis: `執行${strategyName}需要增加固定資產投資，但同時提升企業永續價值，有助改善ESG評等與融資條件。`,
        strategyFeasibilityAnalysis: `考量${companyProfile.sizeText}${companyProfile.industryText}企業資源條件，${strategyName}具備良好執行可行性，建議分階段實施。`,
